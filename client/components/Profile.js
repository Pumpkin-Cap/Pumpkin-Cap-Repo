    ...
    return (
      <Anime duration={6000} opacity={[0, 1]}>
        <div id="profileContainer">
            ...
            {this.props.levels[completedLevels] && (
              <div id="profile-level-link">
                <Link to={`/level/${completedLevels + 1}`}>
                  ...
                  <div style={{ display: "flex", justifyContent: "center" }}>
                    {Array.isArray(levels) && allLevels[levels.length].name}
                  </div>
                </Link>
              </div>
            )}
          ...
          {completedLevels !== 0 && (
            <div style={{ marginBottom: "10px" }}>Levels Completed:</div>
          )}
          <div id="profile-completed-levels">
            <div className="profile-completed-level">
              <div className="profile-completed-title">Control Flow</div>
              {Array.isArray(levels) &&
                levels
                  .filter((element) => element.category === "Control Flow")
                  .map((element) => <div key={element.id}>{element.name}</div>)}
            </div>
            <div className="profile-completed-level">
              <div className="profile-completed-title">Loops</div>
              {Array.isArray(levels) &&
                levels
                  .filter((element) => element.category === "Loops")
                  .map((element) => <div key={element.id}>{element.name}</div>)}
            </div>
            <div className="profile-completed-level">
              <div className="profile-completed-title">Arrays</div>
              {Array.isArray(levels) &&
                levels
                  .filter((element) => element.category === "Arrays")
                  .map((element) => <div key={element.id}>{element.name}</div>)}
            </div>
            <div className="profile-completed-level">
              <div className="profile-completed-title">Objects</div>
              {Array.isArray(levels) &&
                levels
                  .filter((element) => element.category === "Objects")
                  .map((element) => <div key={element.id}>{element.name}</div>)}
            </div>
          </div>
        </div>
